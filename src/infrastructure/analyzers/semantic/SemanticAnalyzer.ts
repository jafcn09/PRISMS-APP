...
export class SemanticAnalyzer extends BaseAnalyzer {
  ...
  private checkUnusedImports(metadata: CodeMetadata): Issue[] {
    const issues: Issue[] = [];
  ...
  private checkUnusedVariables(metadata: CodeMetadata): Issue[] {
    const issues: Issue[] = [];
  ...
  private checkUnreachableCode(ast: AST): Issue[] {
    return [];
  }

  private checkInconsistentNaming(metadata: CodeMetadata): Issue[] {
    const issues: Issue[] = [];
